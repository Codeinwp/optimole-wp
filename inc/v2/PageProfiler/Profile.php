...
class Profile {
    ...
    public function __construct() {
         ...
         */
        $storage_class =  apply_filters('optml_page_profiler_storage', Storage\Transients::class);
    ...
    public static function generate_id( string $content = ''):string{
         ...
         * @return string New id
         */
        return apply_filters('optml_page_profile_id', sha1($content)    );
     ...
     * @param array<string, array<string, array<int, string>>> $af_bg_selectors
	 *     Array structure:
	 *     [
	 *         'css_selector' => [
 ...
 *                                                                   'type' is the type of the LCP element
    ...
    public function store(string $id, string $device_type, array $above_fold_images, $af_bg_selectors = [], $lcp_data = []){
        if(!in_array($device_type, self::get_active_devices())){
            return;
        }
        ...
        $this->storage->store( $id . '_' . $device_type, [
     ...
     *
     * @return string|null The current profile ID or null if not set.
     */
