describe( 'Shareaholic', function () {
	...
	it( 'images should not have quality:eco', function () {
		cy.get( 'img' ).should( ( $imgs ) => {
			...
			expect( $imgs.eq( 5 ) ).to.have.attr( 'src' ).and.to.not.contain( 'eco' )
			expect( $imgs.eq( 6 ) ).to.have.attr( 'src' ).and.to.not.contain( 'eco' )
			expect( $imgs.eq( 7 ) ).to.have.attr( 'src' ).and.to.not.contain( 'eco' )
		} );
	} );
} );