describe( 'Check Homepage', function () {
	...
	it( 'Beaver column should have background lazyloaded', function () {
		cy.get( '.entry-content' ).find( '.fl-col-content' ).eq( 0 ).should( 'have.attr', 'class' ).and( 'include', 'optml-bg-lazyloaded' );
	} );
	it( 'Beaver column should have background lazyloaded', function () {
		cy.get( '.entry-content' ).find( '.fl-col-content' ).eq( 1 ).should( 'have.attr', 'class' ).and( 'include', 'optml-bg-lazyloaded' );
	} );
	it( 'Beaver column should have background lazyloaded', function () {
		cy.get( '.entry-content' ).find( '.fl-col-content' ).eq( 2 ).should( 'have.attr', 'class' ).and( 'include', 'optml-bg-lazyloaded' );
	} );
