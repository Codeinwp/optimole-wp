				...
				<div class="field columns optml-light-background" style="padding: 1% 5%;">
						<div class="field has-addons column has-addons-centered optml-filters-content">
								...
								<p style="margin-left: 2.5%;">
													<span v-if="this.selected_filter===this.FILTER_TYPES.URL" >
																<select v-model="selected_url_match_type" class="optml-text-input-border">
																		<option value="contains">{{this.strings.filter_operator_contains}}</option>
																		<option value="matches">{{this.strings.filter_operator_matches}}</option>
																</select>
														</span>
														<span v-else>{{getFilterOperator()}}</span>
	...
	export default {
		...
		data() {
			return {
				...
				selected_value: '',
				selected_url_match_type: 'contains',
		...
		methods: {
			saveRule: function () {
				...
				let filter = {};
				if (this.selected_filter === this.FILTER_TYPES['URL'] && this.selected_url_match_type === 'matches' ) {
					this.selected_filter = this.FILTER_TYPES['URL_MATCH'];
				}
				filter[this.type] = {};
