<template>
  <div :class="{ 'saving--option' : this.$store.state.loading }">
    ...
    <div class="field  columns" v-if="showManualQuality">
      ...
      <div class="column">
        <vue-slider @change="() => { showSave=true;showSample=true }" v-bind="sliderOptions"
                    v-model="new_data.quality"></vue-slider>
      </div>
    </div>

    <div class="field  is-fullwidth columns optml-settings-desc-margin">
      <div class="column is-left optml-side-by-side">
        ...
        <a v-if="showSample" @click="newSample(false)"
           style="text-decoration: underline !important; margin-top: 5px; font-size:14px !important;"
           class="view-sample-image is-link">{{ strings.view_sample_image }}
        </a>
      </div>
    </div>
